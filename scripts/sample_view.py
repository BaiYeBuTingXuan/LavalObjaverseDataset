...
def main():
    ...
    parser.add_argument("--name", type=str, default='V', help="Base name for output files")
    ...
    parser.add_argument("--output_dir", type=str, default="./view/src", help="Output directory for .npy files")
    parser.add_argument("--target_radius", type=float, default=0.1, 
                        help="Max offset radius for look-at target from origin (0 = always look at center)")
    ...
    while len(poses) < target_count and attempt < max_attempts:
        ...
        z = radius * math.sin(elev_rad)  # Z = vertical axis

        location = mathutils.Vector((x, y, z))

        # Sample look-at target near origin (within target_radius sphere)
        if args.target_radius > 0:
            # Uniform sampling within sphere
            while True:
                # Sample in cube [-r, r]^3
                tx = random.uniform(-args.target_radius, args.target_radius)
                ty = random.uniform(-args.target_radius, args.target_radius)
                tz = random.uniform(-args.target_radius, args.target_radius)
                if tx*tx + ty*ty + tz*tz <= args.target_radius**2:
                    break
            target_loc = mathutils.Vector((tx, ty, tz))
        else:
            target_loc = mathutils.Vector((0, 0, 0))  # Strictly look at origin

        direction = (target_loc - location)
        ...
        if not is_duplicate:
            ...
            npy_path = os.path.join(args.output_dir, f"{args.name}{len(poses)}.npy")
    ...
    # Save JSON
    os.makedirs("./view/info", exist_ok=True)
    json_path = "./view/info/all_views.json"
