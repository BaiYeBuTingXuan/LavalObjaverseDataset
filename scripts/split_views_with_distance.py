...
def main():
    ...
    parser.add_argument("--data_dir", type=str, default='./view/src/', help="Directory containing .npy pose files")
    ...
    with open(f"./view/info/{prefix}full_training_view.json", 'w') as f:
        json.dump(train_files, f, indent=2)
    with open(f"./view/info/{prefix}full_validation_view.json", 'w') as f:
        json.dump(val_files, f, indent=2)
    with open(f"./view/info/{prefix}full_testing_view.json", 'w') as f:
        json.dump(test_files, f, indent=2)
    
    print("\n✅ Split completed. Output files:")
    print(f"  - {prefix}full_training_view.json ({len(train_files)} views)")
    print(f"  - {prefix}full_validation_view.json ({len(val_files)} views)")
    print(f"  - {prefix}full_testing_view.json ({len(test_files)} views)")
    ...
    with open(f"./view/info/{prefix}distance_report.json", 'w') as f:
